#!/usr/bin/env python
import socket
import serial
import base64
import hashlib
import struct
import time
import string
import random
from select import select
ser = serial.Serial('/dev/ttyUSB3',19200,8,'E',1,timeout = 0.1)

class WebSocket():

    @staticmethod
    def handshake(conn):                     #握手应答交互
        key = None 
        data = conn.recv(8192)
        if not len(data):
            return False
        for line in data.split('\r\n\r\n')[0].split('\r\n')[1:]:
            k, v = line.split(': ')
            if k == 'Sec-WebSocket-Key':
                key = base64.b64encode(hashlib.sha1(v + '258EAFA5-E914-47DA-95CA-C5AB0DC85B11').digest())
        if not key:
            conn.close()
            return False
        response = 'HTTP/1.1 101 Switching Protocols\r\n'\
                   'Upgrade: websocket\r\n'\
                   'Connection: Upgrade\r\n'\
                   'Sec-WebSocket-Accept:' + key + '\r\n\r\n'
        conn.send(response)
        return True

    @staticmethod
    def recv(conn, size=8192):
        data = conn.recv(size)
        if not len(data):
            return False
        length = ord(data[1]) & 127
        if length == 126:
            mask = data[4:8]
            raw = data[8:]
        elif length == 127:
            mask = data[10:14]
            raw = data[14:]
        else:
            mask = data[2:6]
            raw = data[6:]
        ret = ''
        for cnt, d in enumerate(raw):
            ret += chr(ord(d) ^ ord(mask[cnt%4]))
        return ret
                      
    @staticmethod
    def send(conn, data):
        head = '\x81'
        if len(data) < 126:
            head += struct.pack('B', len(data))
        elif len(data) <= 0xFFFF:
            head += struct.pack('!BH', 126, len(data))
        else:
            head += struct.pack('!BQ', 127, len(data))
        conn.send(head+data)

server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
socket_list = set()
def sendall(data):
    for sock in socket_list:
        if sock != server:
            WebSocket.send(sock, data)

def process(conn, data): #you should change this method
    WebSocket.send(conn, data)

def main(handle=process):
    port = 8888                                  #需要修改端口
    try:
        server.bind(('192.168.0.131', port))     #需要修改IP
        server.listen(100)                       #监听数量
    except Exception, e:
        print e
        exit()
    socket_list.add(server)
    print 'server start on port %d' % port
    while True:
        r, w, e = select(socket_list, [], [])
        data = ser.read(18)                      #数据采集，单通道模式
       # print 'before:',socket_list
        for sock in r:
            if sock == server:
                conn, addr = sock.accept()
                if WebSocket.handshake(conn):
                    socket_list.add(conn)        #去重监听
                    print 'after:',socket_list
            else:
               # data = WebSocket.recv(sock)        
                #if not data:
                 #   print data
                  #  socket_list.remove(sock)
               # else:
                    #print data
                    #while True:
                     #     data=ser.read(18)
                   try:
                    if data:
                             #data+=str(random.random())
                             print data
                             handle(sock,data)
                   except:
                             socket_list.remove(sock)
                             pass

if __name__ == '__main__':
    main()
